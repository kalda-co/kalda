...
export interface ApiClient {
  ...
  getPostState(postId: number, commentId: number, notificationId: number): Promise<Response<Post>>;
}
...
export class AuthenticatedApiClient implements ApiClient {
  ...
  async getPostState(postId: number, commentId: number, notificationId: number): Promise<Response<Post>> {
    let url = this.route(`/v1/token/posts/${postId}/comments/${commentId}/n/${notificationId}`)
